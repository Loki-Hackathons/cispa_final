...
echo "Job ID: $SLURM_JOB_ID | Node: $SLURM_NODELIST | GPUs: 2"
nvidia-smi --query-gpu=index,name,memory.total --format=csv
echo ""

python -u "$SCRIPT" $ARGS

# In main.py: from job_progress import bind_job, report, complete
# bind_job("task_1", attempt=1); report(step, total, phase) each epoch
