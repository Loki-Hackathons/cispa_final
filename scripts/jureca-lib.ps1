# Shared helpers for JURECA SSH ControlMaster via WSL Ubuntu.
# Dot-source from jureca-connect.ps1 and jureca-ssh.ps1.

$script:JurecaWslDistro = "Ubuntu-26.04"
...
function Invoke-Wsl {
    ...
    try {
        $out = wsl -d $script:JurecaWslDistro -- bash -lc $Script 2>&1
...
function Get-JurecaMasterPid {
    $out = wsl -d $script:JurecaWslDistro -- bash -lc 'ssh -o ControlPath=~/.ssh/sockets/ctl-%r@%h-%p -O check jureca 2>&1' 2>&1 | Out-String
...
function Start-JurecaMaster {
    if (-not $env:TOTP_CODE) {
        Write-Error @"
No SSH master and TOTP_CODE not set. Ask the user for a fresh 6-digit TOTP, then:
  `$env:TOTP_CODE='<code>'; .\scripts\jureca-connect.ps1
"@
        exit 1
    }
